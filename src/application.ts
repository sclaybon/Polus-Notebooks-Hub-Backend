...
import {PolusNotebooksHubBackendApiBindings} from './keys';
export class PolusNotebooksHubBackendApplication extends BootMixin(
  ...
  setUpBindings(options: ApplicationConfig): void {
    this.bind('datasources.config.ApiDS').to(options.polusNotebooksHubBackend?.db);
    this.bind(PolusNotebooksHubBackendApiBindings.CONFIG).to(options);
